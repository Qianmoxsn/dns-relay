...
const upstreamServerIPv6 = config.upstreamDNS.ipv6;

logger.trace(`Find Upstream DNS Server: ${upstreamServer}:${upstreamPort} (IPv4) and ${upstreamServerIPv6}:${upstreamPort} (IPv6) in Config`);
...
async function v6forward(msg) {
    return new Promise((resolve, reject) => {
        ...
        // 将接收到的DNS请求转发到上游DNS服务器
        client.send(msg, 0, msg.length, upstreamPort, upstreamServerIPv6, (err) => {
            logger.info(`[Forward] Send DNS request to ${upstreamServerIPv6}:${upstreamPort}`); // 打印转发请求的日志
            if (err) {
                logger.error(`client error:\n${err.stack}`); // 打印错误信息
                client.close(); // 关闭客户端套接字
                reject(err);
